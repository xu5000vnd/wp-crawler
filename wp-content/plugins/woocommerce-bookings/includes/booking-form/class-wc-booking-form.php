...
class WC_Booking_Form {
			...
			if ( $this->product->has_person_types() ) {
				...
				foreach ( $person_types as $person_type ) {
					...
					$this->add_field( array(
						...
						'totalcost' => $person_type->get_block_cost() + $person_type->get_cost(),
					) );
				}
			} else {
				$this->add_field( array(
	...
	public function output() {
		$this->scripts();
		$this->prepare_fields();
		$string = "<div class='col medium-12'><legend style ='margin: 5px 0 15px;padding-bottom: 15px;'><span class='label'>Loại vé: </span></legend>";
		$date ="<div class='col medium-12'>";
